...
Validate Tail Report Brown Mule
    [Documentation]    สำหรับไฟล์ create myselfe
    Over view File Dark Brown Mule form SFTP Data
    Over view File Dark Brown Mule from Portal Download
    Should Not Be Empty    ${csv_list_sftp}[1]
    Should Not Be Empty    ${csv_list_portal_download}[1]
    Should Be Equal    ${csv_list_sftp}[2]    ${csv_list_portal_download}[2]

Validate Tail Report Brown Mule Have Content
    Over view File Dark Brown Mule form SFTP Data
    Over view File Dark Brown Mule from Portal Download 
    ${csv_list_sftp_tail}=   Evaluate    [ row for row in ${csv_list_sftp} if row[0].startswith('#') ]    modules=builtins
    ${csv_list_sftp_last_tail}=   Evaluate    [ row for row in ${csv_list_sftp} if row[0].startswith('เอกสารฉบับนี้จัดทำขึ้นผ่านระบบ') ]    modules=builtins
    ${csv_list_portal_download_tail}=   Evaluate    [ row for row in ${csv_list_portal_download} if row[0].startswith('#') ]    modules=builtins
    ${csv_list_portal_download_last_tail}=   Evaluate    [ row for row in ${csv_list_portal_download} if row[0].startswith('เอกสารฉบับนี้จัดทำขึ้นผ่านระบบ') ]    modules=builtins
    Should Be Equal    ${csv_list_sftp_last_tail}    ${csv_list_portal_download_last_tail}
    Should Not Be Empty    ${csv_list_sftp_tail}
    Should Not Be Empty    ${csv_list_portal_download_tail}
