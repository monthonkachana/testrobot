...
def sign_and_encrypt_json(private_key_path, data, input_csv_path, output_gpg_path, recipient_key, passphrase):
    try:
        # ตั้งค่าตัวแปร GPG_TTY สำหรับระบบ Unix (Linux/macOS)
        os.environ['GPG_TTY'] = os.popen('tty').read().strip()
        ...
        # สร้างไฟล์ลายเซ็นชั่วคราว
        signature_file_path = "temp_signature.asc"
        with open(signature_file_path, "w", encoding="utf-8") as sig_file:
            sig_file.write(str(signed_data))  # เขียนลายเซ็นลงไฟล์
